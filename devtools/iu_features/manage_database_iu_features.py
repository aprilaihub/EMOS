...
def _row_block_regex(db_id: str) -> re.Pattern[str]:
    return re.compile(
        ...
        rf"<label><input type=\"checkbox\" ui-type=\"database\" value=\"{re.escape(db_id)}\">.*?</label>\s*"
        rf"<button[\s\S]*?data-iu-feature=\"{re.escape(db_id)}\"[\s\S]*?</button>\s*"
        rf"</div>",
        re.S,
    )
...
def _plain_label_regex(db_id: str, display_name: str) -> re.Pattern[str]:
    return re.compile(
        rf"(?m)^\s*<label><input type=\"checkbox\" ui-type=\"database\" value=\"{re.escape(db_id)}\">\s*{re.escape(display_name)}\s*</label>\s*$"
    )
...
def _build_button_row(db_id: str, display_name: str, description: str) -> str:
    return (
        ...
        f"                                <label><input type=\"checkbox\" ui-type=\"database\" value=\"{db_id}\"> {display_name}</label>\n"
        "                                <button\n"
...
def _remove_button_row(index_text: str, db_id: str, display_name: str) -> str:
    row_re = _row_block_regex(db_id)
    plain_label = f'                            <label><input type="checkbox" ui-type="database" value="{db_id}"> {display_name}</label>'
