"""
MattergenMagneticDensityHhiGenerator
=====================================
Multi-property-conditioned model targeting magnetic density and
Herfindahl-Hirschman elemental scarcity index.
Pretrained model: ``dft_mag_density_hhi_score``.
"""
...
class MattergenMagneticDensityHhiGenerator(MattergenGenerator):
    """Thin wrapper that locks the pretrained model to *dft_mag_density_hhi_score*."""

    PRETRAINED_NAME = "dft_mag_density_hhi_score"
    NUM_BATCHES = 1
    RECORD_TRAJECTORIES = False

    def __init__(self, generator_name: str = "mattergen_magnetic_density_hhi", logger=None):
        super().__init__(generator_name, logger)

    # ------------------------------------------------------------------
    def info(self) -> str:
        return (
            "MatterGen: Magnetic Density + HHI — multi-property-conditioned "
            "model targeting magnetic density and Herfindahl-Hirschman "
