// MatterGen Base Model IU Feature (feature-style UI for generator Information Unit)
class MattergenBaseModelIUFeature extends BaseFeature {
    constructor(featureId, iuMeta = {}) {
        super(
            ...
            iuMeta.iuDesc || 'Run MatterGen Base Model generation with predefined IU contracts.'
        );
    ...
    createInputsHTML() {
        return `
            <div class="iu-input-scroll">
                ...
                <div class="input-controls" id="mattergenBaseModelPropertyFilters_${this.featureId}">
    ...
    createOutputsHTML() {
        return `
            <p>MatterGen: Base Model IU outputs</p>
            <div class="output-display" id="outputDisplay_${this.featureId}">
                ...
                <div class="output-item">
                    ...
                    <a id="iuDataGeneratedDownload_${this.featureId}" style="display:none; margin-left:10px;" download="mattergen_base_model_generated_dataset.json">Download</a>
                </div>
            </div>
        `;
    }
    ...
    async _renderPropertyFilters() {
        const container = document.getElementById(`mattergenBaseModelPropertyFilters_${this.featureId}`);
        if (!container) return;

        try {
            const [mappingRes, commonRes] = await Promise.all([
                fetch('./Information_Units/property_mappings/sources/generators/mattergen_base_model.json'),
                fetch('./Information_Units/property_mappings/common_properties.json'),
            ]);
            if (!mappingRes.ok) throw new Error(`MatterGen Base Model mapping HTTP ${mappingRes.status}`);
            ...
            defs.forEach((prop) => {
                ...
                if (prop.rangeSupport) {
                        ...
                        <div class="iu-property-row">
                            <label>${labelWithUnit}
                                <div class="iu-range-inputs">
                                    <input type="number" step="any" id="mgbm_prop_${prop.name}_min_${this.featureId}" placeholder="Min" title="${prop.name}">
                                    <input type="number" step="any" id="mgbm_prop_${prop.name}_max_${this.featureId}" placeholder="Max" title="${prop.name}">
                                </div>
                            </label>
                        </div>
                    `;
                    ...
                    html += `
                        <div class="iu-property-row">
                            <label>${labelWithUnit}
                                <input type="text" id="mgbm_prop_${prop.name}_${this.featureId}" placeholder="Enter Value" title="${prop.name}">
                            </label>
                        </div>
                    `;
                }
            ...
            container.innerHTML = `<p>Failed to load MatterGen Base Model property mappings: ${error.message}</p>`;
            this.addLog(`Failed to load MatterGen Base Model property mappings: ${error.message}`, 'error');
        }
    }
    ...
    collectInputData() {
        ...
        this._propertyDefs.forEach((prop) => {
            if (prop.rangeSupport) {
                const minEl = document.getElementById(`mgbm_prop_${prop.name}_min_${this.featureId}`);
                const maxEl = document.getElementById(`mgbm_prop_${prop.name}_max_${this.featureId}`);
                ...
                const valEl = document.getElementById(`mgbm_prop_${prop.name}_${this.featureId}`);
    ...
    updateOutputs(results = null) {
        ...
        if (downloadEl) {
            ...
            downloadEl.download = `mattergen_base_model_generated_dataset_${Date.now()}.json`;
